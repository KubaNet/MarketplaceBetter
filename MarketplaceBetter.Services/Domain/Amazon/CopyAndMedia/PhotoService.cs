...
namespace MarketplaceBetter.Services.Domain.Amazon.CopyAndMedia
{
    public class PhotoService : IPhotoService
    {
        ...
        private IQueryable<Photo> ApplyFilter(IQueryable<Photo> photos, ListRequest request, bool showSharedOnly)
        {
            ...
            {
                string[] searchFieldNames = new[] { "id", "product", "product_id", "variant", "variant_id", "asin", "size", "color", "instance", "file_name", "type", "kind", "height", "width", "comment", "cloud_id" };
                ...
                if (searchField != null)
                {
                    photos = searchField.Name switch
                    {
                        ...
                        "cloud_id" => photos.Where(p => p.CloudId.Contains(searchField.Value)),
